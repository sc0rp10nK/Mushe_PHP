...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    if(isset($_GET['q'])){
        ...
        if (0 === strpos($_GET['q'], "@") ){
            ...
            $q = "%{$tmq}%";
            $sql = "SELECT * FROM POSTS JOIN USERS ON POSTS.post_userid = USERS.userid WHERE date IS NOT NULL AND content LIKE :q";
            $sth = $db->prepare($sql);
            $sth->bindParam(":q", $q);
            $sth->execute();
    ...
    <body>
            ...
            <?php for ($i = 0; $i < count($resultPosts); ++$i): ?>
                ...
                <div class="post_user_box">
                    ...
                    <p name="post_user_name" id="post_user_name">
                        <?echo h($resultPosts[$i]['name']);?>
                    </p>
                    <p name="post_user_id" id="post_user_id">
                        @<?echo h($resultPosts[$i]['userid']);?>
                    </p>
                    </div>
                    <?php if (
                        isset($_SESSION["username"]) &&
                ...
                <div class="posts_body">
                    <p>
                    <?echo nl2br($resultPosts[$i]['content']);?>
                    </p>
                </div>
                <div class="posts_footer">
                    <p class="posts_date">
                    <?php echo convert_to_fuzzy_time($resultPosts[$i]["date"]); ?>
                    </p>
                </div>
                <a class ="link" href="/p?id=<?echo $resultPosts[$i]['postid']?>">
                </a>
                </div>
            <?php endfor; ?>
        <?php elseif(isset($resultUsers) && count($resultUsers) > 0):?>
            ...
            <?php for ($i = 0; $i < count($resultUsers); ++$i): ?>
                <div class="user_box">
                    <a class ="profile_link" href="/profile/?id=<?echo $resultUsers[$i]["userid"]?>">
                    ...
                    <a class ="profile_link" href="/profile/?id=<?echo $resultUsers[$i]["userid"]?>">
                        ...
                        <form class="followbtn_block" action="../actions/follow.php" method="post">
                            ...
                            value="<?= h(generate_token()) ?>"
                            />
                            <input type="hidden" name="followid" value=<?echo $resultUsers[$i]['userid']?>>
                            <?php if (
                                isFollowed(
                                    $db,
